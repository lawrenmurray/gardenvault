...
const EntranceFlow = {
    ...
    showGardenMessage() {
        const msg = this.createMessage('Many see the garden; one heart is known.');
        dom.container.appendChild(msg);
    ...
    handleRealName(value, input, msg, inputWrapper) {
        const hasSpecialWord = value.toLowerCase().includes('beloved') ||
                               value.toLowerCase().includes('rose');
        ...
        if (hasSpecialWord) {
            ...
            if (validNames.includes(value.toLowerCase())) {
                ...
                setTimeout(() => {
                    dom.container.innerHTML = '';
                    const rejectMsg = this.createMessage('The garden speaks to one.');
    ...
    showFrontInscription() {
        ...
        input.addEventListener('input', () => {
            ...
            typingTimer = setTimeout(() => {
                const value = input.value.trim();
                if (value && value.toLowerCase() === 'only in my dreams') {
    ...
    showBackInscription() {
        ...
        input.addEventListener('input', () => {
            ...
            typingTimer = setTimeout(() => {
                const value = input.value.trim();
                if (value && value.toLowerCase() === 'you can always plan on me') {
    ...
    showWelcomeJessica() {
        ...
        const welcomeText = state.userData.isPearlsEntry ?
            'Welcome back, sweet flower.' : 'Welcome, Jessica.';
        dom.welcomeMessage.textContent = welcomeText;
