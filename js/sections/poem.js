...
const PoemSection = {
    ...
    cleanupAndReturn() {
        ...
        if (window.innerWidth <= 768) {
            ...
            if (bookHelper) {
                bookHelper.style.removeProperty('--keyboard-helper-top');
            }
        ...
        if (bookHelper) {
            bookHelper.style.transition = 'opacity 0.3s ease';
            bookHelper.style.opacity = '0';
        }
        ...
        setTimeout(() => {
            ...
            if (bookHelper) bookHelper.remove();
    ...
    showBook() {
        ...
        prompt.textContent = 'Touch to begin...';
    ...
    createClosedBookSVG() {
        ...
        svg.innerHTML = `
            ...
            <!-- Center book icon (styled like whisper envelope icon) -->
            <circle cx="65" cy="70" r="10" fill="none" stroke="rgba(160, 140, 180, 0.3)" stroke-width="1"/>
            <g transform="translate(57, 62)">
                <path d="M2 9.75A1.25 1.25 0 013.25 8.5H10M2 9.75V2.25A1.25 1.25 0 013.25 1H10v10H3.25A1.25 1.25 0 012 9.75z"
                      fill="none" stroke="rgba(160, 140, 180, 0.4)" stroke-width="0.8" transform="scale(1.3)"/>
            </g>
    ...
    showOpenBook(withOpeningAnimation = false) {
        ...
        leftPage.appendChild(charCount);
        ...
        // Create helper icon (spawns after opening animation)
        setTimeout(() => {
            this.createBookHelper();
        }, withOpeningAnimation ? 1200 : 300);
    ...
    setupTyping(textarea, openBook) {
        ...
        textarea.addEventListener('input', (e) => {
            const now = Date.now();
            const newText = e.target.value.substring(0, this.maxChars);
            const oldLength = state.poem.currentText.length;
            ...
            if (newLength > oldLength) {
                ...
                // Create typing firefly for new chars
                const charPosition = Helpers.getTextareaCharacterPosition(textarea);
                if (charPosition) {
                    Guardian.captureInteraction('typing.feedback', { type: 'firefly', field: 'poem.textarea' });
        ...
        textarea.addEventListener('focus', () => {
            ...
            if (bookHelper) bookHelper.classList.add('input-focused');
            ...
            if (window.innerWidth <= 768) {
                ...
                if (window.visualViewport) {
                    const updatePosition = () => {
                        ...
                        const bookHelperEl = document.getElementById('bookHelper');
                        ...
                        // Calculate scaled book height for centering
                        const scaledBookHeight = bookHeight * scale;
                        ...
                        // Position helper: right side, centered between book bottom and keyboard top
                        if (bookHelperEl) {
                            const bookBottom = centerY + (scaledBookHeight / 2);
                            const helperTop = bookBottom + ((visibleHeight - bookBottom) / 2);
                            // Clamp to reasonable position (not too close to edges)
                            const clampedTop = Math.min(Math.max(helperTop, bookBottom + 30), visibleHeight - 30);
                            bookHelperEl.style.setProperty('--keyboard-helper-top', `${clampedTop}px`);
                        }
                    };
        ...
        textarea.addEventListener('blur', () => {
            ...
            if (bookHelper) bookHelper.classList.remove('input-focused');
    ...
    updateInkDisplay() {
        ...
        // Update char count
        if (charCount) {
            charCount.textContent = `${text.length} / ${this.maxChars}`;
        }
    ...
    setupBookPhysics(openBook) {
        ...
        if (!isMobile) {
            ...
            // Desktop: Left page drag-to-close (like mobile)
            leftPage.addEventListener('mousedown', (e) => {
                ...
                // Only enable drag-to-close if there's content
                if (!hasText) return;
                ...
                const onMouseMove = (moveE) => {
                    if (thresholdReached || !isDraggingToClose) return;

                    const deltaX = moveE.clientX - dragStartX;

                    // Only respond to rightward drag (closing direction)
                    if (deltaX > 10) {
                ...
                const onMouseUp = (upE) => {
                    ...
                    const wasDragging = isDraggingToClose && hasDraggedEnough;
                    isDraggingToClose = false;
    ...
    createBookHelper() {
        ...
        // Animate book icon from book center to corner (like whisper envelope)
        this.animateBookHelperSpawn();
    },

    // Exactly like whisper's animateFirstWhisper - book icon travels to corner
    animateBookHelperSpawn() {
        const openBook = document.getElementById('openBook');
        if (!openBook) {
        ...
        // Get the book icon position (center of open book)
        ...
        // Create traveling book icon (clone style from whisper envelope)
        const travelIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        travelIcon.id = 'travelingBookIcon';
        travelIcon.setAttribute('viewBox', '0 0 24 24');
        travelIcon.setAttribute('fill', 'none');
        travelIcon.setAttribute('stroke', '#e8d4f0');
        travelIcon.setAttribute('stroke-width', '1.5');
        travelIcon.innerHTML = `<path d="M4 19.5A2.5 2.5 0 016.5 17H20M4 19.5V4.5A2.5 2.5 0 016.5 2H20v20H6.5A2.5 2.5 0 014 19.5z"/>`;
        ...
        // Calculate target (bottom right corner - same as whisper)
        setTimeout(() => {
            const isMobile = window.innerWidth <= 768;
            const targetRight = isMobile ? 20 : 30;
            const targetBottom = isMobile ? 20 : 30;

            // Same calculation as whisper
            travelIcon.style.transition = 'all 1.5s cubic-bezier(0.4, 0, 0.2, 1)';
            travelIcon.style.left = `calc(100vw - ${targetRight + 40}px)`;
            travelIcon.style.top = `calc(100vh - ${targetBottom + 40}px)`;
            travelIcon.style.transform = 'scale(0.9)';

            // Shrink and fade at destination (same timing as whisper)
            setTimeout(() => {
                travelIcon.style.transition = 'all 0.3s ease-out';
                travelIcon.style.transform = 'scale(0.5)';
                travelIcon.style.opacity = '0';

                // Firefly burst (same as whisper)
                setTimeout(() => {
                    ...
                    for (let i = 0; i < 8; i++) {
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                firefly.style.left = burstX + 'px';
                                firefly.style.top = burstY + 'px';
                    ...
                    // Create the actual helper button
                    this.createBookHelperElement();

                    // Remove traveling icon
                    travelIcon.remove();
                }, 150);
            }, 1400);
        }, 50);
    ...
    createBookHelperElement() {
        ...
        icon.innerHTML = `<path d="M4 19.5A2.5 2.5 0 016.5 17H20M4 19.5V4.5A2.5 2.5 0 016.5 2H20v20H6.5A2.5 2.5 0 014 19.5z"/>`;
    ...
    showBookHelp() {
        ...
        text.innerHTML = 'Let your words flow like ink upon the page...<br><br>When your heart has spoken,<br>draw the left page toward the right<br>to close the book and send your request.';
    ...
    showReturnConfirmation(callback) {
        ...
        text.textContent = 'Close the tome unfinished?';
    ...
    sendRequest() {
        ...
        if (bookHelper) {
            bookHelper.style.pointerEvents = 'none';
    ...
    showPendingState() {
        ...
        if (bookHelper) {
            bookHelper.style.filter = '';
            // Keep helper hidden during pending
        }
        ...
        // Mini book with orbiting embers
        const pendingBook = document.createElement('div');
        pendingBook.classList.add('pending-book');
        pendingBook.id = 'pendingBook';

        // Orbiting embers
        for (let i = 0; i < 3; i++) {
            const orbitEmber = document.createElement('div');
            orbitEmber.classList.add('orbit-ember');
            pendingBook.appendChild(orbitEmber);
        }

        // Pending book SVG
        const pendingSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        pendingSvg.classList.add('pending-book-svg');
        pendingSvg.setAttribute('viewBox', '0 0 120 140');
        pendingSvg.innerHTML = `
            <defs>
                <radialGradient id="pendingCoverGrad" cx="40%" cy="30%" r="70%">
                    <stop offset="0%" stop-color="rgba(240, 230, 250, 0.92)"/>
                    <stop offset="40%" stop-color="rgba(200, 180, 220, 0.85)"/>
                    <stop offset="100%" stop-color="rgba(160, 140, 180, 0.7)"/>
                </radialGradient>
                <linearGradient id="pendingSpineGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stop-color="rgba(140, 120, 160, 0.85)"/>
                    <stop offset="100%" stop-color="rgba(190, 160, 130, 0.8)"/>
                </linearGradient>
                <linearGradient id="pendingSilkGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stop-color="rgba(120, 35, 45, 0.95)"/>
                    <stop offset="50%" stop-color="rgba(170, 60, 70, 0.85)"/>
                    <stop offset="100%" stop-color="rgba(110, 30, 40, 0.95)"/>
                </linearGradient>
            </defs>
            <path d="M 20 18 Q 16 18, 16 22 L 16 108 Q 16 112, 20 112 L 26 112 L 26 18 Z"
                  fill="url(#pendingSpineGrad)"/>
            <rect x="26" y="18" width="78" height="94" rx="2"
                  fill="url(#pendingCoverGrad)" stroke="rgba(200, 180, 220, 0.75)" stroke-width="1.5"/>
            <rect x="28" y="110" width="74" height="2" rx="0.5" fill="rgba(235, 225, 245, 0.6)"/>
            <rect x="34" y="26" width="62" height="78" rx="1"
                  fill="none" stroke="rgba(200, 180, 220, 0.35)" stroke-width="0.5"/>
            <ellipse cx="65" cy="65" rx="10" ry="12" fill="none" stroke="rgba(160, 140, 180, 0.4)" stroke-width="0.8"/>
            <circle cx="65" cy="65" r="3" fill="rgba(160, 140, 180, 0.25)"/>
            <path d="M 36 112 L 44 112 L 44 130 L 36 126 Z" fill="url(#pendingSilkGrad)"/>
        `;
        pendingBook.appendChild(pendingSvg);

        // Pending text
        const pendingText = document.createElement('div');
        pendingText.classList.add('pending-text');
        pendingText.textContent = 'Enchanting your words...';
    ...
    showPendingMessage() {
        ...
        text.innerHTML = 'Your words already dance<br>within my heart...<br><br>Please let me finish weaving<br>them into verse.';
